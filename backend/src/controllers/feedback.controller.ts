...
const isMongooseValidationError = (
  error: unknown
): error is { errors: Record<string, { message?: string }> } => {
  if (typeof error !== "object" || error === null) return false;

  const maybeError = error as { name?: unknown; errors?: unknown };

  return (
    maybeError.name === "ValidationError" &&
    typeof maybeError.errors === "object" &&
...
export const createFeedback = async (req: Request, res: Response) => {
  try {
    ...
    if (isMongooseValidationError(error)) {
      ...
      return sendError(res, messages.join(", ") || "Validation failed", 400);
    }
